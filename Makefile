...
go-build:
	GOOS=linux CGO_ENABLED=0 go build -o teams ./cmd/main.go

docker-build:
	docker build --no-cache -t lambda-teams:0.0.1 .

run:
	docker run --env-file local.env --rm -p 9000:8080 lambda-teams:0.0.1
...
go-lint:
	docker run -v ${PWD}:/app -w /app $(GOLANG_CI_LINT_IMAGE) \
		go env -w GOPRIVATE=github.com/twizar/common
		golangci-lint run -v --timeout 600m --fix
